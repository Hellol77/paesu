/*******************************************************************************
 * Static folder tree Copyright (C) October 2005 DTHMLGoodies.com, Alf Magne
 * Kalleland This library is free software; you can redistribute it and/or
 ...
 * Street, Fifth Floor, Boston, MA 02110-1301 USA Dhtmlgoodies.com., hereby
 * disclaims all copyright interest in this script written by Alf Magne
 * Kalleland. Alf Magne Kalleland, 2006 Owner of DHTMLgoodies.com
 ******************************************************************************/

/*
 * Update log: December, 19th, 2005 - Version 1.1: Added support for several
 * trees on a page(Alf Magne Kalleland) January, 25th, 2006 - Version 1.2: Added
 * onclick event to text nodes.(Alf Magne Kalleland) February, 3rd 2006 -
 * Dynamic load nodes by use of Ajax(Alf Magne Kalleland)
 */

var idOfFolderTrees = [ 'dhtmlgoodies_tree' ];

var imageFolder = (window['APPLICATION_CONTEXT_ROOT'] || '') + '/resources/images/admin/tree/'; // Path to images
...
var ajaxRequestFile = 'writeNodes.jsp';
...
function Set_Cookie(name, value, expires, path, domain, secure) {
	...
	var cookieString = name + "=" + escape(value) + ((expires) ? ";expires=" + expires_date.toGMTString() : "") + ((path) ? ";path=" + path : "") + ((domain) ? ";domain=" + domain : "")
			+ ((secure) ? ";secure" : "");
...
function addNewNode(e) {
	if (!okToCreateSubNode)
		return;
	setTimeout('okToCreateSubNode=true', 200);
	contextMenuObj.style.display = 'none';
	okToCreateSubNode = false;
	source = contextMenuSource;
	while (source.tagName.toLowerCase() != 'li')
		source = source.parentNode;
	
	/*
	 * if (e.target) source = e.target; else if (e.srcElement) source =
	 * e.srcElement; if (source.nodeType == 3) // defeat Safari bug source =
	 * source.parentNode;
	 */
...
function deleteNode() {
	...
	if (!confirm('Click OK to delete the node ' + source.innerHTML))
		return;
	okToCreateSubNode = false;
	...
	if (lis.length == 0)
		parentLi.getElementsByTagName('IMG')[0].style.visibility = 'hidden';
