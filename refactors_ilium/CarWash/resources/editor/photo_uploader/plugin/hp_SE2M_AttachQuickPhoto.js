...
nhn.husky.SE2M_AttachQuickPhoto = jindo.$Class({		
		...
		try{
			...
			for(var i = 0; i <aPhotoData.length; i++){				
				...
				aPhotoInfo = {
					...
					nWidth: htData.nWidth,					// hyeong 이미지사이즈 (리사이즈 위해)
				    nHeight: htData.nHeight,					// hyeong
				    imgSeq: htData.imgSeq
				};
	...
	_getPhotoTag : function(htPhotoInfo){
		...
		/** hyeong img 리사이즈위해  추가. */
		var maxWidth = 500;
		var sTag;	//화면에 뿌려질 이미지 태그 
		if (htPhotoInfo.nWidth > maxWidth) {	// 이미지 whidth 가 500이상일 경우 resize
			var nResizeWidth = maxWidth;
			var nResizeHeight = (maxWidth * htPhotoInfo.nHeight) / htPhotoInfo.nWidth;
			htPhotoInfo.nWidth = nResizeWidth;
			sTag= '<img src="{=sOriginalImageURL}" class="editorImg" title="{=sName}" '+ 'style="max-width:100%;" '+ 'alt="{=imgSeq}">';
		} else {	// 500 미만일 경우 그대로 출력 
			sTag = '<img src="{=sOriginalImageURL}" class="editorImg" title="{=sName}" '+ 'alt="{=imgSeq}">';
		}
			/** end */
		//var sTag = '<img src="{=sOriginalImageURL}" title="{=sName}" >'; // 기존
