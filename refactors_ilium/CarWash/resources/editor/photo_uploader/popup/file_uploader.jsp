...
String path = "/upload/customer/"; // 이미지가 저장될 주소
String filename = "";
...
<%
	...
	try {
		...
		MultipartRequest multi=new MultipartRequest(request, request.getRealPath(path), 15*1024*1024, "UTF-8", new DefaultFileRenamePolicy());
		...
		if (!upfile.equals("")) {
			...
			File sourceFile = new File(request.getRealPath(path) + File.separator + upfile);
			File targetFile = new File(request.getRealPath(path) + File.separator + moveFileName);
